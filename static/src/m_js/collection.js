...
// 定义操作列,######以下是重点########
            ...
            "render": function (data, type, row) {
                ...
                html += "<i class='delete' style='display: none'></i><a href='javascript:void(0);'    class='btn btn-link btn-danger btn-xs' onclick='cancel_collect(" + id + ',this' +")' ><i class='fa fa-times'></i> 取消收藏</a>"
                return html;
            }
        }],
    ...
    // 初始化刪除按钮
    $('#collectionlist tbody').on('click', 'i.delete', function (e) {
        // alert('del')
        e.preventDefault();
...
//取消收藏该笔记
function cancel_collect(nid, obj) {
    // alert('del2')
   ...
   // var jsonstr = {"action": 'delete_note', "nid": nid,"n_type":n_type};
     ...
     $.ajax({
        type: "GET",
        url: '/api/mgr/note?action=cancel_collect_note&nid='+nid,
        // data: JSON.stringify(jsonstr),//将json对象转换成json字符串发送
        dataType: "json",
        success: function (data) {
            if (data.ret == 0 && data.msg == '取消成功') {
                //console.log('取消收藏成功')
                var preDom = obj.previousElementSibling;
                preDom.click();//成功  明天吧<i>隐藏了
